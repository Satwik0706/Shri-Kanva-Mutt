package com.satwik.example.mutt_app.ui.theme

import androidx.compose.ui.graphics.Color

// Reference Image Palette
val Terracotta = Color(0xFFAC5B2B) // Primary brown/orange
val Parchment = Color(0xFFF9F1E6)  // Light background
val DeepSlate = Color(0xFF2D2D2D)  // Dark text
val Sandstone = Color(0xFFE8D7C3)  // Secondary surface/card
val GoldAccent = Color(0xFFD4AF37) // Accent for icons/logo

// Keeping existing for compatibility where needed
val DeepSaffron = Color(0xFFFF9933)
val Cream = Color(0xFFFFFDD0)
val DeepBrown = Color(0xFF5D4037)
val OffWhite = Color(0xFFFDF5E6)
val DarkOrange = Color(0xFFE65100)
val Golden = Color(0xFFFFD700)
val LightSaffron = Color(0xFFFFCF8B)
